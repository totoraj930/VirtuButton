 ...
 */
export type SelectOption<K extends string> = SelectOptionK<K> | SelectOptionGroup | SelectOptionHr;
/**
 * selectとして表示されます
 ...
 * アクションに登録するプロパティです
 */
export type PluginInputField = StringField | TextField | NumberField | BooleanField | SelectField<any> | KeyCombinationField;
...
export type PluginDynamicFields<T extends PluginFieldMap = PluginFieldMap> = (initValues?: PluginFieldValues<T>) => T | Promise<T>;
/**
 * アクションに登録するプロパティの集まりです
 */
export type PluginFields<T extends PluginFieldMap = PluginFieldMap> = PluginStaticFields<T> | PluginDynamicFields<T>;
/**
 * フロントに送られるFieldsはID
...
export type PluginFieldsToFieldMap<T> = T extends Record<string, PluginInputField> ? T : T extends PluginDynamicFields<any> ? PromiseType<ReturnType<T>> : never;
/**
