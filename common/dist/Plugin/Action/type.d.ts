...
export type PluginAction<T extends PluginFieldMap = PluginFieldMap> = {
    ...
    name: string;
    description?: string;
    run: (fieldValues: PluginFieldValues<T>, payload: ActionPayload) => Promise<any> | any;
    fields: PluginFields<T>;
};
export type PluginActionAdapted<T extends PluginFieldMap = PluginFieldMap> = Omit<PluginAction<T>, 'fields'> & {
    pluginId: string;
    fieldsId: string;
};
export type PluginActionSerialized<T extends PluginFieldMap = PluginFieldMap> = Omit<PluginActionAdapted<T>, 'run'>;
